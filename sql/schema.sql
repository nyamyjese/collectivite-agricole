...
-- 7. VUES UTILES
CREATE VIEW federation.v_membres_actifs AS
SELECT
    m.id            AS membre_id,
    m.nom,
    m.prenoms,
    ...
    a.date_debut
FROM membre m
         JOIN appartenance a ON a.membre_id = m.id
WHERE a.date_fin IS NULL
  AND m.statut = 'ACTIF';


CREATE VIEW v_tresorerie_collectivite AS
SELECT
    c.collectivite_id,
    SUM(CASE WHEN c.type_compte = 'CAISSE'       THEN c.solde ELSE 0 END) AS total_caisse,
    SUM(CASE WHEN c.type_compte = 'BANCAIRE'     THEN c.solde ELSE 0 END) AS total_bancaire,
    SUM(CASE WHEN c.type_compte = 'MOBILE_MONEY' THEN c.solde ELSE 0 END) AS total_mobile_money,
    SUM(c.solde) AS total_general
FROM compte c
WHERE c.est_federation = FALSE
GROUP BY c.collectivite_id;

CREATE VIEW v_assiduite_annuelle AS
SELECT
    p.membre_id,
    a.collectivite_id,
    EXTRACT(YEAR FROM a.date_activite) AS annee,
    COUNT(*)                                            AS total_activites,
    SUM(CASE WHEN p.statut_presence = 'PRESENT' THEN 1 ELSE 0 END) AS presents,
    SUM(CASE WHEN p.statut_presence = 'EXCUSE'  THEN 1 ELSE 0 END) AS excuses,
    SUM(CASE WHEN p.statut_presence = 'ABSENT'  THEN 1 ELSE 0 END) AS absents,
    ROUND(
            100.0 * SUM(CASE WHEN p.statut_presence = 'PRESENT' THEN 1 ELSE 0 END)
                / NULLIF(COUNT(*), 0),
            2) AS taux_assiduite
FROM presence p
         JOIN activite a ON a.id = p.activite_id
WHERE p.est_visiteur = FALSE
GROUP BY p.membre_id, a.collectivite_id, EXTRACT(YEAR FROM a.date_activite);
