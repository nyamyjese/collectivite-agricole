...
public class BankAccountRepository {
    ...
    public BankAccount save(BankAccount bankAccount) {
        ...
        String sql = "INSERT INTO bank_account (account_id, bank, account_number) VALUES (?, ?::bank, ?)";
    ...
    public Optional<BankAccount> findById(String id) {
        String sql = """
            SELECT a.id, a.collectivity_id, a.is_federation, a.account_type,
                   ...
                   bk.bank, bk.account_number
            FROM account a
            JOIN bank_account bk ON bk.account_id = a.id
            WHERE a.id = ?
            """;
        try (Connection conn = db.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
    ...
    public boolean existsByAccountNumber(String accountNumber) {
        String sql = "SELECT COUNT(1) FROM bank_account WHERE account_number = ?";
        try (Connection conn = db.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
