...
public class AccountValidator {
    ...
    public void confirmCreatePayment(CreatePaymentRequest request) {
        ...
        if (accountRepository.fundExist(request.getCollectivityId(), request.isFederation())) {
    ...
    public void validateCreateBankAccount(CreateBankAccountRequest request) {
        ...
        if (request.getAccountNumber() != null) {
            ...
            if (!num.matches("\\d{" + ACCOUNT_NUMBER_LENGTH + "}")) {
                errors.add("The bank account number must contain exactly " + ACCOUNT_NUMBER_LENGTH + " numbers (format: BBBBB GGGGG CCCCCCCCCCC KK). Included: " + num.length() + " characters");
            } else {
